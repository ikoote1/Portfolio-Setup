...
const addClasses = function addClasses(node, classes) {
  return classes && classes.forEach((className) => node.classList.add(className));
};
...
const removeClasses = function removeClasses(node, classes) {
  return classes && classes.forEach((className) => node.classList.remove(className));
};
...
const handleScroll = function handleScroll($elements) {
  return $elements.forEach((el, i) => applyClasses(el, window.pageYOffset));
};
...
const refresh = function refresh() {
  const initialize = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
